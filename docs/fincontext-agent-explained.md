...
## 4. Why AMD Developer Cloud Matters

The hackathon wants meaningful AMD hardware usage. Our hardware story is:

```text
One AMD Developer Cloud VM runs the whole backend.
```

The important model is Qwen2.5-72B. A 70B-class model in FP16 needs roughly
140 GB of GPU memory. An AMD MI300X has 192 GB of HBM3 VRAM, so it can run this
large model on one GPU.

That matters because many 80 GB GPUs cannot run the same 70B FP16 model without
splitting it across multiple GPUs. We can tell judges:

```text
The final memo is generated by a 72B model running on a single AMD GPU.
```

We still use a smaller 14B model for cheaper intermediate tasks, because using
the large model for everything would waste the hackathon credit.
...
Instead of letting every part of the app call vLLM or TEI directly, everything
...
```text
/v1/chat/completions with fincontext-planner  -> 14B model
/v1/chat/completions with fincontext-reasoner -> 72B model
...
- Gateway routes that to the TEI embedding service;
...
Wraps the validation helpers in one command so the AMD one-filing smoke check is
easy to run.
...
- AMD VM is not fully provisioned/validated yet.
- Real model services are not confirmed running on AMD GPU.
...
2. Get AMD VM and Gateway/Qdrant running.
...
vectors before writing them to Qdrant. What is not done yet is a real VM run
...
The AMD GPU is the engine that makes the large model practical for the final
memo.
